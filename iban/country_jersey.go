...
// validateJerseyIBAN validates Jersey IBAN
func validateJerseyIBAN(iban string) error {
	if len(iban) != 22 {
		return &ErrValidationLength{Expected: 22, Actual: len(iban)}
	}
	if subject := iban[0:2]; subject != "JE" {
		return &ErrValidationStaticValue{Position: 0, Expected: "JE", Actual: subject}
	}
...
// generateJerseyIBAN generates Jersey IBAN
func generateJerseyIBAN() (string, error) {
...
// getJerseyBBAN retrieves BBAN structure from Jersey IBAN
func getJerseyBBAN(iban string) (BBAN, error) {
	if len(iban) != 22 {
