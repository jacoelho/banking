...
// validateIsleOfManIBAN validates Isle Of Man IBAN
func validateIsleOfManIBAN(iban string) error {
	if len(iban) != 22 {
		return &ErrValidationLength{Expected: 22, Actual: len(iban)}
	}
	if subject := iban[0:2]; subject != "IM" {
		return &ErrValidationStaticValue{Position: 0, Expected: "IM", Actual: subject}
	}
...
// generateIsleOfManIBAN generates Isle Of Man IBAN
func generateIsleOfManIBAN() (string, error) {
...
// getIsleOfManBBAN retrieves BBAN structure from Isle Of Man IBAN
func getIsleOfManBBAN(iban string) (BBAN, error) {
	if len(iban) != 22 {
