...
func ValidateFinlandIBAN(iban string) error {
    ...
    if subject := iban[10:17]; !ascii.Every(subject, ascii.IsDigit) {
        return fmt.Errorf("range rule, start pos: 10, length: 7, expected type Digit, found %s: %w", subject, ErrValidation)
    }
