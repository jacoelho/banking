// Code generated by banking/registry; DO NOT EDIT.

package iban

import (
	"github.com/jacoelho/banking/ascii"
	"github.com/jacoelho/banking/pool"
)

// validateBulgariaIBAN validates Bulgaria IBAN
func validateBulgariaIBAN(iban string) error {
	if len(iban) != 22 {
		return &ErrValidationLength{Expected: 22, Actual: len(iban)}
	}
	if subject := iban[0:2]; subject != "BG" {
		return &ErrValidationStaticValue{Position: 0, Expected: "BG", Actual: subject}
	}
	if subject := iban[2:4]; !ascii.IsDigit(subject) {
		return &ErrValidationRange{Position: 2, Length: 2, Expected: CharacterTypeDigit, Actual: subject}
	}
	if subject := iban[4:8]; !ascii.IsUpperCase(subject) {
		return &ErrValidationRange{Position: 4, Length: 4, Expected: CharacterTypeUpperCase, Actual: subject}
	}
	if subject := iban[8:14]; !ascii.IsDigit(subject) {
		return &ErrValidationRange{Position: 8, Length: 6, Expected: CharacterTypeDigit, Actual: subject}
	}
	if subject := iban[14:22]; !ascii.IsAlphaNumeric(subject) {
		return &ErrValidationRange{Position: 14, Length: 8, Expected: CharacterTypeAlphaNumeric, Actual: subject}
	}
	if c := checksum(iban); c != iban[2:4] {
		return &ErrValidationChecksum{Expected: c, Actual: iban[2:4]}
	}
	return nil
}

// generateBulgariaIBAN generates Bulgaria IBAN
func generateBulgariaIBAN() (string, error) {
	sb := pool.BytesPool.Get()
	defer sb.Free()
	sb.WriteString("BG")
	ascii.Digits(sb, 2)
	ascii.UpperCaseLetters(sb, 4)
	ascii.Digits(sb, 6)
	ascii.AlphaNumeric(sb, 8)
	return ReplaceChecksum(sb.String())
}

// getBulgariaBBAN retrieves BBAN structure from Bulgaria IBAN
func getBulgariaBBAN(iban string) (BBAN, error) {
	if len(iban) != 22 {
		return BBAN{}, &ErrValidationLength{Expected: 22, Actual: len(iban)}
	}
	return BBAN{BBAN: iban[4:22], BankCode: iban[4:8], BranchCode: iban[8:12], AccountNumber: iban[12:22]}, nil
}
